...
if ! command -v "$GH" >/dev/null 2>&1; then
  if [[ -x /tmp/gh_2.93.0_macOS_arm64/bin/gh ]]; then
...
if git remote get-url origin >/dev/null 2>&1; then
  ...
  echo "GitHub リポジトリ ${REPO_NAME} を作成して push します..."
...
echo "  2. New app → Repository: $("$GH" repo view --json nameWithOwner -q .nameWithOwner)"
